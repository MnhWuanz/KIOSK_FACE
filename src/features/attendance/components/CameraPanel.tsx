...
export const CameraPanel = forwardRef<Webcam, CameraPanelProps>(
        ...
        {cameraError && (
          <div className="absolute inset-0 z-20 grid place-items-center bg-slate-950 p-8 text-center text-white">
            <div className="max-w-md">
              ...
              <Button className="mt-5" type="primary" onClick={onRetryCamera}>
                Thử lại camera
              </Button>
            </div>
          </div>
          ...
          <div className="absolute inset-0 z-10 grid place-items-center bg-slate-900 text-center text-white p-6">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-400 max-w-xs mx-auto">
                Thiết bị tạm thời tắt camera khi ngoài giờ hoặc không có ca điểm
                danh nào mở.
              </p>
            </div>
          </div>
          ...
          <>
            ...
            <div className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-slate-950/90 via-slate-950/45 to-transparent px-5 pb-5 pt-20 text-white sm:px-7 sm:pb-7">
              <div className="flex items-center gap-3">
                <span
                  className={`grid size-10 place-items-center rounded-full ${ready ? 'bg-emerald-500' : 'bg-white/15 backdrop-blur'}`}
                >
                  <IconGlyph>◎</IconGlyph>
                </span>
                <div>
                  <p className="m-0 text-[11px] font-bold uppercase tracking-[0.16em] text-slate-300">
                    Hướng dẫn
                  </p>
                  <p className="m-0 mt-1 text-base font-bold sm:text-lg">
                    {instruction}
                  </p>
                </div>
              </div>
